<template>
  <div class="col-md-3 side-bar">
          ...
          <div class="panel-body">
              <div class="users-label">
                  <router-link v-for="(user,index) in activeUsers" :key="index" :to="`/${user.name}`" class="users-label-item">
                    <img :src="user.avatar" alt="用户照片" class="avatar-small inline-block">
                    {{ user.name }}
                  </router-link>
              </div>
          </div>
      ...
      <!-- 七天内最热 -->
      <div class="panel panel-default corner-radius panel-active-topics">
          <div class="panel-heading text-center">
              <h3 class="panel-title">七天内最热</h3>
          </div>
          <div class="panel-body">
              <ul class="list">
                  <li v-for="(article,index) in horArticles" :key="index">
...
export default {
    ...
    created () {
        ...
        this.$axios.post('/articles/hot', {num:10}).then((response) => {
            this.horArticles = response.data
        })
    }
}
</script>
