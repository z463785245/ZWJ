...
Mock.mock('/users/active','get', () =>{
    ...
    articles.map((article) => {
        const artcleComments = Array.isArray(article.comments) ? article.comments : []
        // 合并评论，等价于 comments = comments.concat(articleComments)
        comments = [...comments,...artcleComments]
    })
...
Mock.mock('/articles/hot','post',(options) =>{
    ...
    //取7天评论最多的文章
    let horArticles = articles.filter((article) => (new Date() - new Date(article.date) < 604800000))
    //文章条数
    let num
    ...
    horArticles = horArticles.slice(0, num || 10)
